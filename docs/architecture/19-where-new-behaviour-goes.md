...
| Require a human before a tool runs | a `tool_policy` on `attach_approval_gate` (tool names or `low`/`medium`/`high` risk bands) — the same middleware the `signoff` policy uses |
